...
| 이름   | 역할 설명 |
|--------|-----------|
| 유경아 | 사용자 도메인 및 인증 로직(JWT) 구현, 로그인/회원가입 API 개발, OCR 이미지 업로드 및 글자 추출 처리, 영수증 분석 로직 및 저장 API 구현 |
| 김현수 | DB 스키마 및 전체 구조 설계, 소비 통계 분석 쿼리 및 API 구현, 챌린지 도메인 설계 및 리워드 로직 구현, OCR 결과 기반 소비 판단 처리 |
